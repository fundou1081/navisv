...
### DesignGraph 错误模型
- **问题**: Agent 不能区分“真的没路径”和“工具没追到”
- **方案**: 路径追踪返回 `{result, status, error}` 结构
  - `status`: `found` / `not_found` / `uncertain`
  - `uncertain` 时附带原因（如“slang-netlist 拼接表达式限制”）
- **优先级**: 🔴

### 代码重构
- **问题**: constraint_parser.py / graph_builder.py 超过 700 行，嵌套深
- **方案**:
  - AST 遍历和图构建分离
  - 每个解析器独立模块
  - 抽取公共 AST 遍历工具
...
### 增量构建
- **问题**: 每次 build() 从零解析，大项目慢
- **方案**: AST 不变时复用解析结果，加缓存机制
- **优先级**: 🟡

### 多文件自动发现
- **问题**: 传入顶层文件时找不到子模块
- **方案**: 自动扫描同目录下的 SV 文件，或解析 include 指令
- **优先级**: 🟡
