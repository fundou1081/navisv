...
- [ ] **APB PRDATA 路径**: 为 `assign` 语句生成反向驱动边，使读数据路径可追踪
- [ ] **FSM 内部路径**: 完善 always 块内的状态转换路径建模
- [ ] **组合逻辑出边**: 修复组合赋值信号的出边生成逻辑

#### P1: Agent 可用性增强

- [ ] **条件分支可视化**: 在路径中标注 `condition` 分支信息
  - 例: `data_a → [mode==0] selector_out → [mode==1] pipe1`
- [ ] **CDC 标注**: 自动识别并标注跨时钟域路径
  - 输出增加 `cross_clock_domain: true` 属性
- [ ] **路径置信度**: 增加 `path_confidence` 分数 (0-1)
  - 考虑: 节点匹配度、边完整性、跨模块损失
